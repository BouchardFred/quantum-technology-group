#!/usr/bin/env python3
"""
Fetch publication metadata for Quantum Technology Group author pages.
...
def main() -> None:
    if not SOURCES.exists():
        raise SystemExit("Missing data/publication-sources.json.")
    ...
    for slug, src in sources.items():
        ...
        if not author_id and src.get("orcid"):
            author_id = find_openalex_author_id_from_orcid(src.get("orcid", ""))

        if not author_id:
            print(f"No OpenAlex author ID found for {src.get('name', slug)}")
            continue

        print(f"Fetching publications for {src.get('name', slug)} using OpenAlex author {author_id}")
    ...
    pubs = sorted(all_pubs.values(), key=lambda p: (str(p.get("year", "")), p.get("title", "")), reverse=True)
